...
export default function NodeActionClient({
  node,
  teams,
}: {
  node: any;
  ...
  const isEnemy =
    node.controlledBy?.id && node.controlledBy?.name !== selectedTeam;
                  ...
                  <>
                    <Crosshair
                      className={`w-8 h-8 mb-1 ${
                        holding ? "animate-spin text-white" : "text-zinc-500"
                      }`}
                    />
                    <span
                    ...
                    <div className="flex items-center gap-2 text-[10px] uppercase opacity-70 mt-1">
                      <span
                        className={`w-2 h-2 rounded-full ${
                          holding ? "bg-red-500 animate-ping" : "bg-red-900"
                        }`}
                      ></span>
